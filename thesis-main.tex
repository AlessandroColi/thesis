...
Dopo la prima fase di documentazione e Design spiegata precedentemente si è passati a sviluppare e scrivere il codice effettivo.
Durante questa fase ci si è concetrati sul modulo \texttt{jvmNativeMain} e conseguentemente sui target Native e \ac{JVM}, lasciando momentaneamente \ac{JS} in secondo piano.

Per lo sviluppo di questo modulo ci si è appoggiati alla liberia KMQTT, la quale fornisce un implementazione e relativa di un client \ac{MQTT} in Kotlin. 
In particolare fornisce tra metodi per le principali azioni di \ac{MQTT}:
\begin{itemize}
    ...
    e le opzioni relative all'utilizzo di \ac{MQTT} over WebSocket. Richiede inoltre come paramentro la funzione da eseguire ogni qualvolta il client 
    riceve un messaggio su uno qualsiasi dei topic a cui è iscritto. Connessione poi chiusa tramite il metodo \texttt{disconnect}
    ...
    \item \texttt{subscribe}: metodo che permette al client di iscriversi a uno o più topic, per ognuno specificando le opzioni di iscrizione 
    relative al singolo topic quali il livello di \ac{QoS} per la ricezione dei messaggi. 
\end{itemize}

Poiché si è scelto di utilizzare la libreria KMQTT, che per funzionare su piattaforme native dipende a sua volta dall'utilizzo di OpenSSL, che è specifico per ogni piattaforma. 
è stato necessario incorporare le implementazioni specifiche per ogni piattaforma all'interno del progetto in una struttura apposita(texttt{src/nativeLibs}). 
Questo è stato realizzato aggiungendo le dipendenze ai file specifici ai singoli target nativi per i singoli sourceset all'interno del file di configurazione Gradle, 
consentendo così di specificare la dipendenza dal file corretto per ciascuna architettura. 

Il modulo sviluppato per i target \ac{JVM} e native sfrutta le courutines di Kotlin per permettere un'esecuzione parallela del client \ac{MQTT} al gestore delle 
comunicazioni tra le varie entità, questo infatti riceve tramite il builder il \texttt{CoroutineDispatcher}. Necessario per la corretta creazione ed esecuzione del 
\texttt{CourutineScope}, con la funzione di ospitare ed eseguire il client per l'invio e la ricezione effettivi dei pacchetti \ac{MQTT}. 
