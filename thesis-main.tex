...
\title{Analisi e Implementazione in Kotlin Multiplatform di un modulo di comunicazione MQTT e integrazione in PulvReAKt per Sviluppo di Applicazioni IoT Eterogenee}
...
\begin{abstract}	
    ...
    Questa tesi descrive l'intero processo di progettazione e sviluppo di un progetto Kotlin Multiplatform, partendo dall'analisi dei requisiti, 
    comprendendo le fasi di progettazione, sviluppo, validazione e infine la fase di integrazione all'interno di PulvReAKt. 

    Il Progetto si propone di gestire le comunicazioni tra molteplici entità, offrendo loro la possibilità di mandare messaggi specificando il destinatario 
    e di leggere messaggi da un mittente specifico. Per offrire tali funzionalità il Progetto sfrutta il protocollo \ac{MQTT} e gestisce autonomamente i topic 
    al suo interno in modo tale da garantire di operare come atteso dall'utente.

    Il Progetto è stato sviluppato autonomamente e può essere usato con leggeri adattamenti all'interno di qualsiasi progetto Kotlin Multiplatform interessato alla funzionalità 
    offerte dal progetto per uno o più target tra quelli per cui è stato sviluppato (\ac{JVM}, Native e NodeJS). 
    Il progetto è stato anche direttamente integrato nel più ampio contesto per la pulverizzazione di sistemi del progetto \texttt{PulvReAKt}.
    
    Viene infine proposto un confronto tra le prestazioni del progetto sulle diverse piattaforme target, con il fine di mostrare come il progetto performi 
...
\section{Sistemi IoT eterogenei} %TODO rivedi
...
L'obiettivo principale dell'\ac{IoT} è migliorare l'automazione, l'efficienza e la gestione dei dati in tempo reale, rendendo gli oggetti intelligenti e interconnessi %\cite{Gubbi2013}.
...
Ad esempio, dispositivi IoT possono utilizzare protocolli come \ac{MQTT} a seconda delle loro necessità e funzioni %\cite{Al-Fuqaha2015}. 
...
La gestione efficace dei sistemi \ac{IoT} eterogenei richiede l'implementazione di soluzioni middleware. Questi strati software agiscono come intermediari tra i dispositivi 
...
diversa natura possano operare insieme in modo coerente e affidabile, supportando così l'espansione e l'adattamento continuo di queste reti intelligenti %\cite{Bandyopadhyay2011}.
...
L'insieme di tutte queste caratteristiche è quello che ha portato alla scelta di utilizzare questo protocollo enl progetto proposto, %TODO collegati a Sistemi IoT eterogenei
...
Il centro del progetto sviluppato è il protocollo \acf{MQTT}, in quanto viene utilizzato per l'invio dei messaggi nella rete.
Questo protocollo è di tipologia publish-subscribe, nella quale i dispositivi connessi si dividono in due ruoli principali: 
i textbf{publisher}, che inviano i messaggi su specifici topic, e i textbf{subscriber}, che si iscrivono a questi topic per ricevere i messaggi pertinenti. 
...
\bibliographystyle{alpha} %TODO: sistema la bibliografia
