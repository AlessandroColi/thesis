...
\begin{abstract}	
ancora da fare tutto, cosa ci va esattamente? Max 2000 characters, strict.
\end{abstract}
...
\textbf{Codice Condiviso}: In questo approccio, il codice condiviso viene scritto una volta e viene compilato per eseguire su diverse piattaforme.
...
\textbf{Interfacce di Programmazione (API)}: Un altro approccio consiste nel definire un'API comune che fornisce un'interfaccia uniforme
...
\textbf{Virtualizzazione e Contenitori}: Utilizzando tecnologie di virtualizzazione e contenitori, è possibile creare un ambiente uniforme 
...
\textbf{Massimizzazione della Condivisione del Codice}: Condividendo il codice tra le diverse piattaforme, 
è possibile ridurre il lavoro ripetitivo e aumentare l'efficienza dello sviluppo.

\textbf{Riduzione dei Costi di Sviluppo}: Riducendo il numero di linee di codice da scrivere e gestire, 
è possibile ridurre i costi complessivi di sviluppo e manutenzione dell'applicazione.

\textbf{Coerenza e Compatibilità}: Mantenendo una sola base di codice per tutte le versioni dell'applicazione, 
è possibile garantire una maggiore coerenza e compatibilità tra le diverse versioni.

\textbf{Flessibilità}: Utilizzando approcci multiplatform, gli sviluppatori possono adottare una varietà di tecnologie e 
...
\textbf{Protocolli di Comunicazione Universali}: Questi protocolli consentono la comunicazione tra dispositivi e sistemi eterogenei,
...
\textbf{Middleware Distribuito}: Il middleware distribuito fornisce un livello di astrazione tra le applicazioni e l'infrastruttura sottostante,
...
\textbf{Containerizzazione}: Le tecnologie di containerizzazione, come Docker e Kubernetes, consentono di confezionare,
...
\textbf{Orchestrazione Multi-Cloud}: L'orchestrazione multi-cloud permette di distribuire carichi di lavoro su più fornitori di servizi cloud,
...
\textbf{Constructor Injection}: Le dipendenze vengono fornite tramite il costruttore dell'oggetto. 
    Questo metodo garantisce che le dipendenze siano disponibili al momento della creazione dell'oggetto.

\textbf{Setter Injection}: Le dipendenze vengono fornite tramite metodi setter. 
    Questo approccio consente una maggiore flessibilità nella configurazione delle dipendenze dopo la creazione dell'oggetto.
  
\textbf{Interface Injection}: L'oggetto client espone un'interfaccia che permette al container di fornire le dipendenze. 
...
JavaScript per lo sviluppo della parte \ac{JS}, integrata comunque all'interno di Kotlin grazie alla sua interoperabilità con JavaScript.
...
JavaScript è uno dei linguaggi di programmazione più diffusi al mondo, utilizzato principalmente per lo sviluppo web. La sua versatilità e la vasta gamma di librerie e framework 
disponibili lo rendono una scelta popolare per una varietà di applicazioni, dai siti web interattivi alle applicazioni server-side.

JavaScript è un linguaggio di programmazione dinamico, interpretato e ad alto livello, con il supporto per la programmazione orientata agli oggetti, 
la programmazione funzionale e la programmazione imperativa. Alcune delle caratteristiche principali di JavaScript includono:

\begin{itemize}
    \item \textbf{Event-driven e Asincrono:} JavaScript è progettato per gestire eventi in modo efficiente e supporta l'esecuzione asincrona, 
    che è essenziale per lo sviluppo web moderno.
    \item \textbf{Interpretabile dal Browser:} JavaScript è nativamente supportato da tutti i principali browser web, permettendo di eseguire script direttamente sul lato client.
    \item \textbf{Interoperabilità:} JavaScript può essere facilmente integrato con Kotlin oltre

\end{itemize}

\section{Piattaforme}

In questo capitolo, analizzeremo le caratteristiche delle tre piattaforme principali su cui è possibile implementare \ac{MQTT} utilizzando Kotlin: \ac{JVM},
 \ac{JS} e nativo. Questo ci permetterà di comprendere i punti di forza e le limitazioni di ciascuna piattaforma.
...
\textbf{Node.js} rappresenta una potente piattaforma per lo sviluppo backend, estendendo le capacità di JavaScript oltre il browser e offrendo un ambiente efficiente e scalabile 
...
\textbf{Event-driven e Non-blocking I/O}: Node.js utilizza un modello event-driven e non-blocking per le operazioni di I/O, consentendo di gestire un numero elevato di connessioni
 concorrenti con un singolo thread grazie al meccanismo delle promise in \ac{JS}.

\textbf{Ecosistema di Moduli }: Node.js ha un vasto ecosistema di moduli e pacchetti disponibili tramite npm (Node Package Manager). 
...
\chapter{Conclusione e Miglioramenti Futuri}
\label{chap:Conclusione e Miglioramenti Futuri}
