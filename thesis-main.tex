...
\begin{abstract}	
    ...
    Il Progetto si propone di gestire in maniera ottimale le comunicazioni tra molteplici entità, offrendo loro la possibilità di mandare messaggi specificando il destinatario 
    ...
    Il Progetto è stato sviluppato autonomamente e può essere inserito, con leggeri adattamenti, all'interno di qualsiasi progetto Kotlin Multiplatform interessato alla funzionalità 
    offerte dal progetto per uno o più target tra quelli per cui è stato sviluppato (\ac{JVM}, Native e NodeJS). 
    Nell'ambito di questa tesi, per supportarne il più ampio utilizzo, il progetto è stato anche direttamente integrato in un contesto più ampio del progetto PulvReAKt.
    
    Infine, con lo scopo di dare un'evidenza pratica delle prestazioni del progetto sviluppato nelle diverse condizioni di lavoro, 
    in questa tesi viene riportato anche un confronto tra i tempi di esecuzione sulle diverse piattaforme interessate. 
    Questo semplice dato mostra in maniera chiara come il progetto performi in diverse condizioni e può quindi rappresentare un elemento di scelta nel caso di futura 
    integrazione in nuovi progetti.
...
\section{PulvReAKt}
 ...
 in muyltemplici componenti modulari che possono essere distribuiti su diverse piattaforme \cite{DBLP:journals/fi/CasadeiPPVW20}. 
