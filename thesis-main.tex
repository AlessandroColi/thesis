...
\usepackage{verbatim} % TODO TOGLI alla fine
...
\begin{comment}
La fase iniziale di sviluppo si è concetrata sul modulo \texttt{jvmNativeMain} e quindi sui target Native e \ac{JVM}, lasciando momentaneamente \ac{JS} in secondo piano.
Durante questa fase sono stati definiti

Tuttavia, poiché KMQTT dipende dall'utilizzo di OpenSSL, che è specifico della piattaforma, è stato necessario incorporare le implementazioni 
specifiche per ogni piattaforma all'interno del progetto in una struttura apposita. Questo è stato realizzato aggiungendo le dipendenze ai file specifici 
ai singoli target nativi come sourceset all'interno del file di configurazione Gradle, consentendo così di specificare la dipendenza dal file 
corretto per ciascuna architettura.
