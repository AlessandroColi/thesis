...
\begin{abstract}	
    ...
    comprendendo le fasi di progettazione, sviluppo e validazione. 
    ...
    Il Progetto è stato sviluppato autonomamente e può essere usato in qualsiasi progetto Kotlin Multiplatform interessato alla funzionalità su uno o più target 
    tra quelli per cui è stato sviluppato (\ac{JVM}, Native e NodeJS). Il progetto è stato anche direttamente integrato nel più ampio contesto per la pulverizzazione di sistemi 
    del progetto \texttt{PulvReAKt}.
    
    Viene infine proposto un confronto tra le prestazioni del progetto sulle diverse piattaforme interessate, con il fine di mostrare come il progetto performi 
...
La tesi spiega nel dettaglio le diverse fasi di sviluppo del progetto, dall'analisi dei requisiti alla validazione.
Il progetto si propone di sviluppare un gestore di comunicazioni che sfrutta il Protocollo \acf{MQTT} ed è stato concepito per funzionare su \acf{JVM}, 
\acf{JS} e native, infine viene descritta l'integrazione del progetto nel più ampio progetto \texttt{PulvReAKt}. 
Il progetto è in grado di gestire le comunicazioni tra molteplici entità, rendendo la metodologia di comunicazione e la logica interna trasparenti all'utente.
Tali entità possono comunicare tra loro semplicemente appoggiandosi al progetto, che gestisce invece la logica e la comunicazione effettiva.

\section{struttura della tesi}
Di seguito è riportata la struttura della tesi che descrive il progetto realizzato in ogni sua parte:
