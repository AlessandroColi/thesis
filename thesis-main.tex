...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \addplot[fill=green] coordinates{({\shortstack{Windows\\Environment}},8.200) ({\shortstack{Linux\\Environment}},4.633)};
        \addplot[fill=blue] coordinates{({\shortstack{Windows\\Environment}},1.865) ({\shortstack{Linux\\Environment}},0.920)};
        \addplot[fill=red] coordinates{({\shortstack{Windows\\Environment}},0.235) ({\shortstack{Linux\\Environment}},0.190)};
    ...
    \end{tikzpicture}\begin{tikzpicture}
        ...
        \addplot[fill=green] coordinates{({\shortstack{Windows\\Environment}},7.593)({\shortstack{Linux\\Environment}},4.853)};
        \addplot[fill=blue] coordinates{({\shortstack{Windows\\Environment}},1.590)({\shortstack{Linux\\Environment}},1.076)};
        \addplot[fill=red] coordinates{({\shortstack{Windows\\Environment}},0.413)({\shortstack{Linux\\Environment}},0.120)};
        \legend{JVM, JS, NATIVE}
        \end{axis}
    \end{tikzpicture}
    \caption{tempi di esecuzione medi nelle diverse condizioni}
    \label{fig:tempi di esecuzione medi}
\end{figure}
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \begin{axis}[
            ...
            nodes near coords,
            enlarge y limits={abs=1.5cm},
            legend style={at={(0.5,1.1)},anchor=north,legend columns=6},
            point meta=explicit symbolic,
            reverse legend,
            yticklabel style={align=left, text width=4em, inner sep=0pt},
            every node near coord/.append style={font=\tiny, anchor=east}
        ]

        \addplot[fill=lime] coordinates {(8.200,{locale\\Windows}) (4.633,{locale\\Linux}) (7.593,{remoto\\Windows}) (4.853,{remoto\\Linux})};
        \addplot[fill=green] coordinates {(9.324,{locale\\Windows}) (4.986,{locale\\Linux}) (9.972,{remoto\\Windows}) (5.559,{remoto\\Linux})};
        
        \addplot[fill=cyan] coordinates {(1.865,{locale\\Windows}) (0.920,{locale\\Linux}) (1.590,{remoto\\Windows}) (1.076,{remoto\\Linux})};
        \addplot[fill=blue] coordinates {(2.335,{locale\\Windows}) (1.108,{locale\\Linux}) (2.039,{remoto\\Windows}) (1.183,{remoto\\Linux})};
        
        \addplot[fill=orange] coordinates {(0.235,{locale\\Windows}) (0.190,{locale\\Linux}) (0.430,{remoto\\Windows}) (0.307,{remoto\\Linux})};
        \addplot[fill=red] coordinates {(0.278,{locale\\Windows}) (0.246,{locale\\Linux}) (0.863,{remoto\\Windows}) (0.390,{remoto\\Linux})};

        \legend{JVM min, JVM max, JS min, JS max, NATIVE min, NATIVE max}
        \end{axis}
    \end{tikzpicture}
...
I risultati delle diverse esecuzioni dei test nelle diverse condizioni, sono rappresentati graficici soprastanti che ne evidenziano i differenti tempi d'esecuzione 
...
\begin{itemize}
    ...
    Nel primo caso vi è una differenza di 3.153 secondi, nel secondo caso di 730 millisecondi mentre in caso di esecuzione nativa di solamente 75ms. 
    Questo mostra come Linux sia più efficiente di Windows nella gestione delle risorse, la differenza sarebbe probabilmente meno notevole 
    in caso di esecuzione su un dispositivo con maggiori capacità hardware.
    ...
    Le differenze nei tempi di esecuzione sono sempre al di sotto dei 300 millisecondi, con l’eccezione del caso di esecuzione sulla \ac{JVM} ospitata in windows,
    in cui in caso il broker sia ospitato localmente il tempo di esecuzione medio è di 8.200s mentre in caso sia ospitato da un server remoto è di 7.593s,
    con una differenza quindi di 607ms che risulta quindi anch'essa pressochè trascurabile. 
    ...
    \item L’elemento che influenza maggiormente le prestazioni è rappresentato dalla piattaforma su cui viene eseguito il codice, il che emerge chiaramente quando si confrontano 
    ...
    È estremamente evidente che quando il codice viene eseguito tramite la \ac{JVM} le prestazioni sono notevolmente peggiori rispetto agli altri due target, 
    anche se la differenza tra native e \ac{JS} è di 1s invece che 6s, è comunque un discostamento non indifferente se si considerano i tempi in questione.

    \item Qualora il codice venga eseguito sulla \ac{JVM} i tempi di esecuione sono notevolmente meno stabili con una differenza di 2 secondi 
...
Che in PulvReAKt è gestito tramite il framework kodein\footnote{
...
}, è stato infatti necessario implementare la funzione \texttt{setupInjector(kodein: DI)} per rendere la classe utilizzabile all'interno di PulvReAKt.

è stato inoltre necessario svolgere altre modifiche minori necessarie per conformare il progetto e la qualità del codice agli standard proposti da PulvReAKt,
questo ha compreso una ridefinizione della documentazione delle interfacce e dei test. Per renderle più chiare e congruenti allo stile proposto all'interno di PulvReAKt.

La componente che ha richiesto più attenzione durante l'integrazione del progetto all'interno di PulvReAKt è stata la struttura di gradle, in particolare è stato necessario
aggiornare il file \texttt{build.gradle} per adattarlo ad essere relativo ad un modulo all'interno di un progetto più ampio anzichè essere per un progetto intero. 
Questo ha richiesto un'attenta analisi della gestione di gradle all'Interno di PulvReAKt, che ha permesso di evitare duplicazioni di codice, 
mantenendo però tutte le funzionalità necessarie. La CI di PulvReAKt ha inoltre evidenziato la mancanza di \texttt{linuxArm64} tra i target nativi 
del progetto gradle, che è stato prontamente aggiunto alla build gradle senza problemi.
