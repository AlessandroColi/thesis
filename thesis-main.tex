...
\subsection{Scalabilità e Affidabilità}
...
efficace dovrebbe prevedere meccanismi di auto-scaling, dove i nuovi nodi vengono aggiunti automaticamente in risposta a picchi di carico e ridotti durante i periodi di bassa domanda, 
ottimizzando l'uso delle risorse e minimizzando i costi operativi. Deve inoltre essere in grado di gestire l'elasticità dell'infrastruttura, permettendo il provisioning e 
de-provisioning dei nodi in maniera automatizzata e senza interruzioni.
...
ogni messaggio venga ricevuto dal destinatario previsto. Ciò implica la gestione di conferme di ricezione, meccanismi di ritrasmissione in caso di fallimento e gestione 
delle code di messaggi per evitare perdite. Per raggiungere questo livello di affidabilità, è stato deciso di utilizzare il livello di Quality of Service (QoS) 1 di \ac{MQTT}. 
QoS 1 garantisce che un messaggio venga consegnato almeno una volta al destinatario, richiedendo una conferma di ricezione (acknowledgement) da parte del ricevente. 
Se il mittente non riceve la conferma, ritrasmette il messaggio fino a quando non viene confermato, riducendo così il rischio di perdita di messaggi in condizioni di rete instabili.

Un altro aspetto fondamentale nella comunicazione tra i vari componenti è l'affidabilità, che prevede il garantire che il modulo di comunicazione sia in grado di gestire 
scenari di errore in modo robusto. Ciò include la gestione di connessioni instabili o perdute, la ri-trasmissione di messaggi non confermati e la gestione delle situazioni 
di sovraccarico della rete. Inoltre, il modulo dovrebbe essere in grado di registrare eventi e attività di comunicazione per scopi di diagnostica e analisi, 
consentendo agli amministratori di sistema di identificare e risolvere eventuali problemi in modo tempestivo.
...
In quanto il progetto si colloca all'interno del contesto di PulvReAKt richiede la capacità di operare su diverse piattaforme, inclusi ambienti \ac{JVM}, JavaScript e nativi.
