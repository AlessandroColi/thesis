...
\item \textbf{Protocolli di Comunicazione Universali}: Questi protocolli consentono la comunicazione tra dispositivi e sistemi eterogenei,
 fornendo un'interfaccia standardizzata per lo scambio di dati e messaggi. Esempi di protocolli universali includono \ac{HTTP}, \ac{MQTT} e gRPC,
...
Nel vasto panorama dell' \ac{IoT}, dove milioni di dispositivi sono interconnessi per scambiare dati e informazioni in tempo reale, \ac{MQTT}  si distingue come 
uno dei protocolli di comunicazione più importanti e ampiamente adottati. In quanto si presenta come una soluzione elegante per risolvere le sfide di comunicazione 
proposte in ambito \ac*{IoT}. Le sue caratteristiche distintive includono il basso utilizzo di banda e CPU, che lo rendono particolarmente adatto per i dispositivi
con risorse limitate, e il modello di comunicazione publish-subscribe, che consente la trasmissione efficiente dei messaggi tra i partecipanti alla rete.
...
Il progetto si focalizza sulla gestione delle comunicazioni all'interno di un sistema distribuito ed eterogeneo, 
dove le componenti possono essere dislocate in diverse posizioni e possono variare in termini di capacità e risorse disponibili. 
Questo sistema deve essere in grado di operare in contesti molto diversi, tra cui l'ambito \ac{IoT}, dove le macchine dispongono di risorse limitate.
Il sistema deve essere in grado di funzionare correttamente anche in ambienti con una connettività di rete instabile o variabile.

Uno degli obiettivi fondamentali del progetto è garantire che le comunicazioni avvengano in modo efficiente e affidabile, assicurando che i messaggi vengano recapitati solo ai 
destinatari corretti. Questo implica l'implementazione di meccanismi di gestione dei messaggi che consentano di indirizzarli in modo preciso e di evitare sprechi di risorse.
...
Include librerie per la gestione del DOM\footnote{
    ...
    tra client e server utilizzando operazioni \ac{HTTP} standard.
